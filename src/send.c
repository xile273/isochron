...
static int prog_poll_txtstamps(struct isochron_send *prog, int timeout)
{
	...
	case SCM_TSTAMP_SCHED:
		uint32_t num_sched_ts = __be32_to_cpu(send_pkt->num_sched_ts);
		if (swts_utc && num_sched_ts < ISOCHRON_LOG_NUM_SCHED_TS) {
			send_pkt->sched_ts[num_sched_ts] = swts;
			send_pkt->num_sched_ts = __cpu_to_be32(num_sched_ts + 1);
		}
		break;
	case SCM_TSTAMP_SND:
		if (swts_utc)
